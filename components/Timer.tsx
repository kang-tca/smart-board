  ...
  return (
    ...
    >
        <div className={
            ...
            {!isFullscreen && <span className="font-semibold text-gray-700 text-lg">Timer</span>}
             <button 
                onClick={onClose} 
        ...
        >
            {isEditing ? (
                 <input
                    ...
                    className={`bg-transparent text-center w-full font-mono outline-none border-b-2 border-gray-500 focus:border-blue-400 ${
                        isFullscreen ? 'text-white text-[20vw] lg:text-[25rem] leading-none' : 'text-6xl'
                    }`}
                />
            ) : (
        ...
        <div className={`grid grid-cols-3 gap-3 mt-4 w-full ${isFullscreen ? 'max-w-md' : ''}`}>
            ...
            >
                {isActive ? 'Pause' : (time === 0 ? 'Start' : 'Start')}
            </button>
            <button
                onClick={handleReset}
